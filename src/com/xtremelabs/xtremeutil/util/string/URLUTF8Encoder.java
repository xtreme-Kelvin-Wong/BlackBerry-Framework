    ...
    for (int i = 0; i < len; i++) {
      ...
      if ('A' <= ch && ch <= 'Z') {		// 'A'..'Z'
        ...
        sbuf.append((char)ch);
	       ...
	       sbuf.append(hex[ch].toUpperCase());
      } else if (ch <= 0x07FF) {		// non-ASCII <= 0x7FF
	       sbuf.append(hex[0xc0 | (ch >> 6)].toUpperCase());
	       sbuf.append(hex[0x80 | (ch & 0x3F)].toUpperCase());
      } else {					// 0x7FF < ch <= 0xFFFF
	       sbuf.append(hex[0xe0 | (ch >> 12)].toUpperCase());
	       sbuf.append(hex[0x80 | ((ch >> 6) & 0x3F)].toUpperCase());
	       sbuf.append(hex[0x80 | (ch & 0x3F)].toUpperCase());
      }
    }
